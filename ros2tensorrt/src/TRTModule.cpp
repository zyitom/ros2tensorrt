...
cudaError_t err;
//最头疼的一段，debug了快两周 funvidia 12.1
// 分配内存
err = cudaMalloc(&device_buffer[input_idx], input_sz * sizeof(float));
if (err != cudaSuccess) {
...
// 复制内存
err = cudaMemcpyAsync(device_buffer[input_idx], x.data, input_sz * sizeof(float), cudaMemcpyHostToDevice, stream);
if (err != cudaSuccess) {
